...
| **Any party may be a long-zero address** | ⚠️ **Corrected 2026-09-09.** This row said an *ED25519* key was what produced a long-zero address, and named only the recipient. The condition is having **no EVM alias**, which is a property of how the account was created and not of its key type — the ECDSA buyers `accounts:create` makes with `setKeyWithoutAlias` are long-zero too. `0x00…0<num>` is then the account's address: a valid value target, and the `msg.sender` it calls this contract as. So it binds the **payer** as well, and there it binds harder — `claimRefund` matches `deposit.payer` against `msg.sender`, so a deposit recorded under a key-derived address is unrefundable by the account that paid it. The address written into a deposit must be the one the network holds |
