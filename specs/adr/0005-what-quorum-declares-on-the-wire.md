...
|---|---|
| `scheme` | *"Payment scheme identifier"*. §6: schemes *"define how payments are formed, validated, and settled"* |
| `extra.paymentFlow` | §6.1: *"**when** settlement occurs relative to resource execution"* — `authorization`, `upfront`, `escrow` |
...
3. **A second `accepts[]` entry**, plain `exact`, describing the identical payment, declaring
   `paymentFlow: "upfront"`, and naming the withholding in its `description`.
...
**Ordering changed, so the flow changed.** §6.1 defines flows by where `/verify` and `/settle` sit
around resource execution, and requires `extra.paymentFlow` to be present whenever the resolved
...
**Proposing a value for a protocol-reserved key is done in the open.** §6.1 reserves
...
- **The fallback entry is accurate about timing and silent about conditionality.** `upfront` is a
  true statement about when funds commit and cannot express that the resource is conditional. The
  `description` carries it in prose, which no client is obliged to read. This is the residual
  weakness of offering a fallback at all, and it is the price of the interoperability it buys.
...
- **The Hedera `exact` binding declares neither an `assetTransferMethod` nor a default flow**, so
  ...
  not politeness here; §6.1 requires it once the resolved flow is not `authorization`.
