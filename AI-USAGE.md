...
| 2026-09-11 | The signed-receipt open question, and a comment upstream (`specs/quorum-scheme.md`) | Searched x402's tracker before drafting anything, which is what found that the facilitator-signed attestation §12 asks for has been open as [#1802](https://github.com/x402-foundation/x402/issues/1802) since March, with a converged design and the author's PRs waiting on a maintainer decision - the first recommendation, formed from issue titles alone, had been to file a new one. Verified the load-bearing claim against `scheme_exact_hedera.md` at `main` rather than against this repository's paraphrase of it, and read §7 to check the cross-reference instead of guessing a section number. Then corrected the open question itself, which claimed a signed receipt would leave the coordinator problem "gone, not mitigated" - wrong, because a facilitator that signs falsely can make a hold credit a deposit that never arrived | Chose to contribute to the existing thread rather than open a competing issue next to [#2222](https://github.com/x402-foundation/x402/issues/2222), and posted the comment under their own name after review. Cut the draft by a third first, and required it to drop this repository's house style: the three points kept are the ones only a native-transfer binding puts in view. Declined to spend the same day condensing the whole `quorum` scheme into a second upstream proposal |
...
**2026-09-11 - recommended filing an issue that had been open for five months, on the strength of
titles alone.** Asked whether §12's signed-settlement-receipt question was worth raising with x402,
the first answer was yes, open a new one, and here is how to distinguish it from the four adjacent
proposals. That came from an issue *list* - titles, dates, comment counts - which showed a genuinely
crowded attestation space and supported the advice perfectly well. Opening
[#1802](https://github.com/x402-foundation/x402/issues/1802) itself showed something the list could
not: the same object already proposed, a maintainer engaged in March, the shape agreed by April, and
the author's PRs waiting since on an architecture call. A new issue would have duplicated a live
thread, filed by an account with [#2222](https://github.com/x402-foundation/x402/issues/2222)
already open in the same repository.

The failure is one this section records twice already - a conclusion asserted before the source was
opened, and corrected only by opening it. What makes this instance worth its own entry is that the
intermediate evidence was sound. The titles were accurate, the crowding was real, and the
differentiation advice would have been right had the thread not existed. **A search result is not a
source**, and the distance between the two was one API call.
